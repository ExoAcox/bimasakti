...
const NavigationPanel = ({ controlRef }: { controlRef: React.RefObject<OrbitControls> }) => {
    ...
    useFrame(() => {
        if (controlRef.current && universe !== "milky_way") {
            ...
            panelRef.current.style.visibility = isMaxZoomOut ? "visible" : "hidden"
        }
    });
    ...
    return <Html fullscreen>
        <div ref={panelRef} className="absolute left-1/2 -translate-x-1/2 bottom-4 flex flex-col text-sm text-secondary bg-background min-w-sm max-w-2xl rounded-md pb-6 pt-4 px-7 z-50 backdrop-blur-sm">
            <div>
